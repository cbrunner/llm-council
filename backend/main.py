...
async def send_message_stream(conversation_id: str, request: SendMessageRequest):
    ...
    async def event_generator():
        try:
            ...
            stage1_results, stage1_failed = await stage1_collect_responses(request.content, web_search=web_search)
            yield f"data: {json.dumps({'type': 'stage1_complete', 'data': stage1_results})}\n\n"

            # Stage 2: Collect rankings
            yield f"data: {json.dumps({'type': 'stage2_start'})}\n\n"
            stage2_results, label_to_model, stage2_failed = await stage2_collect_rankings(request.content, stage1_results)
...
async def serve_spa(full_path: str):
    """Serve the SPA frontend for production."""
